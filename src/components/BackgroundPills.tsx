...
export function BackgroundPills({ shells, agents }: BackgroundPillsProps) {
  ...
  return <View style={styles.backgroundWrap}>
    <View style={styles.backgroundBar}>
      ...
      {!!shells.length && <Pressable style={styles.backgroundPill} onPress={() => setExpanded((value) => value === 'shell' ? null : 'shell')}><ToolIcon name="bash" color={colors.textSecondary} /><Text style={styles.backgroundPillText}>Shell · {shells.length}</Text></Pressable>}
    </View>
    {expanded === 'agent' && <View style={styles.backgroundPanel}>{agents.map((agent) => <View key={`${agent.delegationId}-${agent.index}`} style={styles.backgroundRow}><SpinIcon size={iconSize.pill} /><Text numberOfLines={2} style={styles.backgroundText}>{agent.title}{agent.currentTool ? ` · ${agent.currentTool}` : ''}</Text></View>)}</View>}
    {expanded === 'shell' && <View style={styles.backgroundPanel}>{shells.map((shell) => <View key={shell.id} style={styles.backgroundTask}><View style={styles.backgroundRow}><SpinIcon size={iconSize.pill} /><Text numberOfLines={2} style={styles.backgroundText}>{shell.command}</Text></View>{!!shell.output && <Text numberOfLines={4} style={styles.backgroundOutput}>{shell.output}</Text>}</View>)}</View>}
  </View>;
}
